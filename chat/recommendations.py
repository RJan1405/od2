...
class ContentRecommender:
    ...
    def _get_user_interactions(self):
        ...
        if self._liked_omzo_ids is None:
            ...
            self._liked_user_ids = set(
                liked_omzos.values_list('user_id', flat=True))

            # Get user IDs whose content user has disliked (to penalize similar creators)
            disliked_omzos = Omzo.objects.filter(
                id__in=self._disliked_omzo_ids)
            self._disliked_user_ids = set(
                disliked_omzos.values_list('user_id', flat=True))
    ...
    def _get_scribe_interactions(self):
        ...
        if self._liked_scribe_ids is None:
            ...
            liked_scribes = Scribe.objects.filter(
                id__in=self._liked_scribe_ids)
            self._liked_scribe_user_ids = set(
                liked_scribes.values_list('user_id', flat=True))

            # Get user IDs whose scribes user has disliked (to penalize similar creators)
            disliked_scribes = Scribe.objects.filter(
                id__in=self._disliked_scribe_ids)
            self._disliked_scribe_user_ids = set(
                disliked_scribes.values_list('user_id', flat=True))
    ...
    def get_omzo(self, limit=50):
        ...
        # Candidate Generation: Get recent omzo (e.g., last 90 days) to keep query fast
        cutoff = timezone.now() - timezone.timedelta(days=180)
        from django.db.models import Q
        # Base filter: Public accounts OR Accounts I follow OR My Own accounts
        base_filter = Q(user__is_private=False) | Q(
            user_id__in=following_ids) | Q(user=self.user)
    ...
    def get_explore_feed(self, limit=100):
        ...
        qs = Omzo.objects.filter(
            user__is_private=False, created_at__gte=cutoff).select_related('user')
        ...
        for omzo in candidates:
            ...
            age_days = max(
                1.0, (now - omzo.created_at).total_seconds() / 86400.0)
            ...
            score = math.log1p(trending_score + 1.0) * \
                20.0 + freshness - diversity
    ...
    def get_scribes(self, following_users, limit=20):
        ...
        for scribe in candidates:
            ...
            engagement_score = (scribe.num_likes * 2.0) + \
                (scribe.num_comments * 4.0)
